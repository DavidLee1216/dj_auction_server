...
from api.cart.models import CartUnit
from api.cart.serializers import CartUnitSerializer
...
class CartView(APIView):
    ...
    def get(self, request):
        if not bool(request.user.is_anonymous):
            ...
            cart_units = Session.objects.get(
                session_key=request.session.session_key).cart_units.all()
    ...
    def post(self, request):
        ...
        if not bool(request.user.is_anonymous):
            ...
            cart_unit_data['session'] = Session.objects.get(
